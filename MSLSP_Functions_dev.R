...
getMasksS10 <-function(qaName) {
  ...
  #Out of the image value for S10 is 0 Set mask to 1
  mask[qaM == 0] <- TRUE
  qaM[is.na(qaM)] <- 0
  ...
  #if cloud shadow, adjacent to cloud, cloud, or cirrus, label as 1
  mask[(qaM == 2) | (qaM == 3) | (qaM == 8) | (qaM == 9) | (qaM == 10) | (qaM == 7) | (qaM == 1)] <- TRUE
...
ApplyMask_QA <-function(imgName, tile, waterMask, chunkStart, chunkEnd, params, deleteInput=F, DEBUG=FALSE) {
  ...
  if(unlist(strsplit(imgName_strip,''))[1] == 'T'){
    sensor <- 'S10'
    tileTxt <- unlist(strsplit(imgName_strip, '_', fixed = T))[1]
    date <- substr(unlist(strsplit(imgName_strip, '_', fixed = T))[2], 1, 8)
    julianDate <- as.numeric(format(as.Date(strptime(date, format="%Y%m%d")), "%Y%j")) #formatting the date in the same way as HLS
    outBase <- paste0('S2_S10_', tileTxt, '_', julianDate)
  }else{
...
getIndexQuantile <- function(chunk, numPix, yr, errorLog, params, DEBUG=FALSE) {
  ...
  log <- try({
    ...
    if(params$SCC$runS10 == T){
      imgList <- list.files(path=chunkFold, pattern=glob2rx("S2_*.Rds"), full.names=F)
    }
    else{
...
runTopoCorrection <- function(imgName, groups, slopeVals, aspectVals, chunkStart,chunkEnd, errorLog, params, writeImages=FALSE, slope=NULL, DEBUG=FALSE) {
  ...
  if(sensor == 'S10'){
    splitPath <- unlist(strsplit(imgName, '/'))
    fullPath <- paste(splitPath[1:length(splitPath)-1], collapse = '/')
    fullPath <- paste(fullPath, '/', sep='')
    metDatPath <- paste(fullPath, 'MTD_TL.xml', sep = '')
    parsed <- xmlParse(metDatPath)
    root <- xmlRoot(parsed)
    zMean <- as.numeric(xmlValue(root[['Geometric_Info']][['Tile_Angles']][['Mean_Sun_Angle']][['ZENITH_ANGLE']]))
    sunzenith <- (pi/180) * zMean
    
    aMean <- as.numeric(xmlValue(root[['Geometric_Info']][['Tile_Angles']][['Mean_Sun_Angle']][['AZIMUTH_ANGLE']]))
...
runPhenoChunk <- function(chunk, numPix, imgYrs, phenYrs, errorLog, params, DEBUG=FALSE) {
  ...
  for (i in 1:numPix) {pheno_mat[i,] <- DoPhenologyHLS(b2[i,],  b3[i,],  b4[i,],  b5[i,],  b6[i,], b7[i,],  vi[i,],
                                                       snowPix[i,],dates, imgYrs, phenYrs, splineStart, splineEnd, numDaysFit, pheno_pars, errorLog)}
...
calculateWeights <- function(smoothMat_Masked, numDaysFit, numYrs, pheno_pars) {
  ...
  for (y in 1:numYrs) {
    ...
    sub_vi <- as.matrix(smoothMat_Masked[ind,])
